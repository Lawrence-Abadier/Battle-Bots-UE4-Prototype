...
#include "SpellSystem/FireSpell.h"
#include "AOEFireSpell.generated.h"

/**
  * AAOEFireSpell is the base class for a spell to be spawned in the level.
  * AAOEFireSpell objects spawn at mouse hit location, leaving a volume behind
  * that damages all enemies that stand in it. Enemies that stand in the volume
  * also receive an ignite for X durations. The duration of the ignite gets reset
  * to full duration as long as the enemy is within the volume.
  */
UCLASS()
class BATTLEBOTS_API AAOEFireSpell : public AFireSpell
{
  GENERATED_BODY()
